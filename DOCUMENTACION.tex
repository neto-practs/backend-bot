...
{\color{azulneto}\rule{\linewidth}{1.5pt}}\\[0.6em]
{\Huge\bfseries\color{azulneto} Documentación Técnica}\\[0.5em]
{\large\color{gray} Chatbot Desguace \textbullet{} Backend + Plugin WordPress}\\[0.4em]
{\color{azulneto}\rule{\linewidth}{1.5pt}}

\vspace{3.5cm}
\begin{tabular}{rl}
  \textbf{Proyecto:} & neto-backend \\[0.5em]
  \textbf{Versión:}  & 1.0 \\[0.5em]
  \textbf{Fecha:}    & Junio 2026 \\[0.5em]
  \textbf{Autor:}    & Estudio Neto \\
\end{tabular}

\vspace{3.5cm}
\begin{minipage}{0.78\linewidth}
\begin{tcolorbox}[colback=grisclaro, colframe=grismedio, fontupper=\small]
  \centering
  Referencia técnica del chatbot para desguaces: arquitectura del sistema,
  API del backend, proceso de alta de nuevos clientes e integración del plugin WordPress.
\end{tcolorbox}
\end{minipage}
\end{titlepage}
...
neto-backend es el servidor Node.js que procesa los mensajes del chatbot para desguaces de vehículos. Su función principal es:

\begin{itemize}[noitemsep]
  \item Entender lenguaje natural y extraer datos estructurados (pieza, marca, modelo, año, referencia OEM).
  \item Consultar el catálogo de piezas del desguace a través de su API.
  \item Devolver los resultados al frontend en formato JSON.
\end{itemize}

No almacena datos de usuarios ni tiene base de datos propia: es un intermediario entre el chat y la API del catálogo del desguace.
...
El bot tiene respuestas predefinidas para las preguntas comerciales más habituales. Estas respuestas se configuran en \texttt{src/config/prompts.js} y no requieren conexión a ningún sistema externo.
...
\begin{tikzpicture}[node distance=0.6cm and 1.5cm]
  ...
  \node[nodoverde, minimum width=3.5cm, minimum height=1.1cm, right=of p2] (p3) {Paso 3\\{\small Plugin: configurar}};
  \draw[flecha] (p1) -- (p2);
  \draw[flecha] (p2) -- (p3);
...
Para vaciar la caché manualmente: \quad \verb|curl -X POST .../api/chat/cache/flush -H "x-cache-key: TU_KEY"|
...
\begin{tcolorbox}[colback=moradoclaro, colframe=moradooscuro, fontupper=\normalsize\bfseries,
                  title={\large\bfseries\color{moradooscuro} Sección del equipo Frontend}]
Las secciones siguientes documentan el plugin WordPress y la integración del widget de chat,
redactadas por el equipo de frontend.
\end{tcolorbox}
...
function aplicarPlaceholders(texto, config) {
  ...
  const mapa = {
    ...
    "[[CAMPA]]":             botonUrl(config.campaUrl, "Campa"),
  };
  let resultado = texto;
...
Cada tarjeta muestra la imagen de la pieza, el título y el precio, con un botón ``Ver pieza'' que abre la URL en nueva pestaña. El backend limita la respuesta a las primeras piezas encontradas; el campo \texttt{metadata.totalReal} contiene el total real y permite mostrar al usuario un texto del tipo ``Mostrando 4 de 12 resultados''.

\begin{tcolorbox}[colback=grisclaro, colframe=grismedio, fontupper=\small]
  \textbf{Consulta de ejemplo:} \textit{``busco un mando volante vw golf 2019''} --- la respuesta incluye más tarjetas de las que caben en una sola captura.
\end{tcolorbox}
\begin{figure}[H]
\centering
\adjustbox{max width=\linewidth}{\includegraphics{img_piezas.png}}
\caption{Tarjetas de piezas devueltas por la búsqueda (resultado parcial)}
\end{figure}
